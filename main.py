...
def is_date_in_season(date_str: str, season_start: datetime, season_end: datetime) -> bool:
...
def calculate_date_range(start_date_str, end_date_str=None):
    ...
    try:
        ...
        raise SystemExit(f"Invalid DATE_START format: {start_date_str}. Expected YYYY-MM-DD")
    ...
    if end_date_str:
        try:
            ...
            raise SystemExit(f"Invalid DATE_END format: {end_date_str}. Expected YYYY-MM-DD")
    else:
    ...
    if start_date > end_date:
        raise SystemExit(f"DATE_START ({start_date}) cannot be after DATE_END ({end_date})")
...
def combine_payroll_totals(contract_totals: dict, salary_totals: dict,
                          contract_df: pd.DataFrame, salary_df: pd.DataFrame) -> pd.DataFrame:
...
def load_resort_config():
    ...
    try:
        ...
        raise SystemExit(f"RESORT_GROUP_NUM must be a valid integer, got: {resort_group_num_str}")

    return {
        "resortName": resort_name,
        "dbName": resort_db_name,
        "groupNum": group_num
    }

def load_procs():
    procs_str = os.environ.get("PROCS", "revenue,payroll,payroll_salary,payroll_history,budget,visits,weather")
...
def main():
    ...
    try:
        ...
        raise SystemExit(f"Invalid ACTIVE_PAYROLL_DATE format: {active_payroll_date}. Expected YYYY-MM-DD")
    ...
    webhook_url = os.environ.get("ERROR_WEBHOOK_URL", "https://n8n-v2.mcp.hyperplane.dev/webhook/data-ingestion-errors")
